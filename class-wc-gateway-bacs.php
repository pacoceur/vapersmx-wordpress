...
class WC_Gateway_BACS extends WC_Payment_Gateway {
	...
	private function bank_details( $order_id = '' ) {
		...
		if ( ! empty( $bacs_accounts ) ) {
			...
			if ( $has_details ) {
				echo '<section class="woocommerce-bacs-bank-details"><h2 class="wc-bacs-bank-details-heading">' . esc_html__( 'Cuentas bancarias', 'woocommerce' ) . '</h2>' . wp_kses_post( PHP_EOL . $account_html ) . '<div class="row"><h3 class="comprobante-titulo">Si no envías tu comprobante de pago tu pedido no será procesado</h3>Si no has realizado tu pago, guarda este enlace para enviar el comprobante más tarde https://vapersmx.com/comprobantes</div><div class="row"><a href="https://vapersmx.com/comprobantes" target="_blank"><br /><button>Envía tu comprobante de pago</button></a></div></section>';
			}
		}
