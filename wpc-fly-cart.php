...
! defined( 'WOOFC_VERSION' ) && define( 'WOOFC_VERSION', '2.0.4' );
			...
			class WPCleverWoofc {
							...
							<?php if ( $active_tab === 'settings' ) { ?>
                                <form method="post" action="options.php">
                                    ...
                                    <table class="form-table">
                                        ...
                                        <tr>
                                            <th><?php esc_html_e( 'Seguir comprando', 'woofc' ); ?></th>
                                            <td>
                                                ...
                                                <span class="description">
											<?php esc_html_e( 'Show the Seguir comprando button at the end of fly cart?', 'woofc' ); ?>
										</span>
                                            </td>
                                        </tr>
                                        <tr>
                                            <th><?php esc_html_e( 'Seguir comprando URL', 'woofc' ); ?></th>
                                            <td>
                                                <input type="url" name="_woofc_continue_url" class="regular-text code"
                                                       value="<?php echo get_option( '_woofc_continue_url', '' ); ?>"/>
                                                <span class="description">
											<?php esc_html_e( 'Custom URL for "Seguir comprando" button. By default, only close the fly cart when clicking on this button.', 'woofc' ); ?>
										</span>
                                            </td>
                                        </tr>
                                        <tr>
				...
				function woofc_remove_item() {
					if ( isset( $_POST['cart_item_key'] ) ) {
						...
						$cart = array( 'action' => 'remove_item', 'cart_item_key' => $_POST['cart_item_key'] );
						echo json_encode( $cart );
				...
				function woofc_get_cart_area() {
					...
					if ( is_array( $items ) && ( count( $items ) > 0 ) ) {
						...
						foreach ( $items as $cart_item_key => $cart_item ) {
							if ( ! isset( $cart_item['bundled_by'] ) && apply_filters( 'woocommerce_widget_cart_item_visible', true, $cart_item, $cart_item_key ) ) {
								...
								$cart_html          .= '<div data-key="' . $cart_item_key . '" class="' . esc_attr( apply_filters( 'woocommerce_cart_item_class', 'woofc-item', $cart_item, $cart_item_key ) ) . '"><div class="woofc-item-inner">';
								...
								$cart_item_remove = '<span class="woofc-item-remove" data-key="' . $cart_item_key . '" data-name="' . esc_attr( $woofc_product->get_name() ) . '"><span class="hint--left" aria-label="' . esc_attr__( 'Remove', 'woofc' ) . '"><i class="woofc-icon-icon10"></i></span></span>';
						...
						$cart_html .= '<div class="woofc-no-item">' . esc_html__( 'No hay productos en el carrito.', 'woofc' ) . '</div>';
					}
					...
					if ( get_option( '_woofc_buttons', '01' ) === '01' ) {
						$cart_html .= '<div class="woofc-action"><div class="woofc-action-inner"><div class="woofc-action-left"><a href="' . wc_get_cart_url() . '">' . esc_html__( 'Carrito', 'woofc' ) . '</a></div><div class="woofc-action-right"><a href="' . wc_get_checkout_url() . '">' . esc_html__( 'Finalizar compra', 'woofc' ) . '</a></div></div></div>';
					} else {
						if ( get_option( '_woofc_buttons', '01' ) === '02' ) {
							$cart_html .= '<div class="woofc-action"><div class="woofc-action-inner"><div class="woofc-action-full"><a href="' . wc_get_cart_url() . '">' . esc_html__( 'Carrito', 'woofc' ) . '</a></div></div></div>';
						}

						if ( get_option( '_woofc_buttons', '01' ) === '03' ) {
							$cart_html .= '<div class="woofc-action"><div class="woofc-action-inner"><div class="woofc-action-full"><a href="' . wc_get_checkout_url() . '">' . esc_html__( 'Finalizar compra', 'woofc' ) . '</a></div></div></div>';
						}
					}

					if ( get_option( '_woofc_continue', 'yes' ) === 'yes' ) {
						$cart_html .= '<div class="woofc-continue"><span class="woofc-continue-url" data-url="' . esc_url( get_option( '_woofc_continue_url', '' ) ) . '">' . esc_html__( 'Seguir comprando', 'woofc' ) . '</span></div>';
					}
